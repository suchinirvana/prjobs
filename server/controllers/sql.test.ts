...
const user_meta = (sequelize, DataTypes) => {
    const attributes = {
		id : {type: DataTypes.INTEGER, allowNull: false, autoIncrement:true, primaryKey:true},
...
// user_meta.associate = (models) =>{
//     user_meta.belongsTo(user,{
//         forreignkey : {
//             allowNull : false
//         }
//     })
...
const userMeta =  await db.User.findAll({
    where: {
        id: id
    },
    attributes: {
    ...
    include: {
        model: db.UserMeta,
        attributes:['company_name', 'company_email']
    }
});