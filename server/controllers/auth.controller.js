...
const getById = async (req, res) => {
  try {
    ...
    const user = await db.User.findByPk(id, {
      attributes: { exclude: ["password", "createdAt", "updatedAt","reset_token","reset_token_expiry","verification_code"] },
    });
    ...
    //$sql = `SELECT company_name, company_email, company_address, company_phone, phone, ircc_id, user_image_url FROM tbl_user WHERE user_id = ${id}`;
    //const result = await db.conn.query($sql);
    if (user) {
      res.status(200).send({
        user: user,
...
const changePassword = async (req, res) => {
  try {
    const { currentpass, newpassword, confpassword } = req.body;
    const id = req.params.id;
